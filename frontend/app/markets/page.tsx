...
const MOCK_MARKET: MarketData = {
  ...
  kpis: [
    { label: "Median Price", value: "$350k", delta: 2.3 },
...
export async function fetchMarketData(market: string): Promise<MarketData | null> {
  ...
  if (!base) {
    ...
    return { ...MOCK_MARKET, marketName: `${MOCK_MARKET.marketName} (${market})` };
  }
...
export async function generateMetadata({ params }: { params: { market: string } }): Promise<Metadata> {
  return { title: `${params.market} Market | Web3 Real Estate` };
}

export default async function MarketPage({ params }: { params: { market: string } }) {
  const marketSlug = params.market;
  const data = await fetchMarketData(marketSlug);
